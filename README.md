...
- **POST** to `H_URL` with the full identity header set (API 1) on every request, plus a
  `User-Agent` — the machine's real one (urlmon on Windows, registry fallback), a static
  browser string only where no system UA exists (NOT optional: Cloudflare's Browser
  Integrity Check 403s urllib's default `Python-urllib/x` signature — error 1010 — before
  the request ever reaches the worker); body =
